...
export default function DataProgram() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            dataToSubmit.append('jadwal_hari', formData.jadwal_hari);
            dataToSubmit.append('waktu', formData.waktu);
            ...
            if (formData.id) {
                ...
                await programService.update(formData.id, dataToSubmit);
            } else {
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-heading font-bold text-gray-900">Data Program</h1>
                    <p className="text-muted-foreground">Kelola data program kerja dan kegiatan.</p>
                </div>
                {!isFormVisible && (
            ...
            {isFormVisible && (
                <Card className="animate-in slide-in-from-top-4 duration-300">
                    <CardHeader>
                        <CardTitle>{formData.id ? 'Edit Program' : 'Input Program Baru'}</CardTitle>
                        <CardDescription>Masukkan detail program kerja.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    <Label htmlFor="judul">Nama Program</Label>
                                    <Input id="judul" value={formData.judul} onChange={handleInputChange} required placeholder="Contoh: Jumat Curhat" />
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="jadwal_hari">Jadwal Hari</Label>
                                    <Input id="jadwal_hari" value={formData.jadwal_hari} onChange={handleInputChange} placeholder="Contoh: Setiap Jumat" />
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="waktu">Waktu</Label>
                                    <Input id="waktu" value={formData.waktu} onChange={handleInputChange} placeholder="Contoh: 09:00 - Selesai" />
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="lokasi">Lokasi</Label>
                                    <Input id="lokasi" value={formData.lokasi} onChange={handleInputChange} placeholder="Contoh: Bergilir per Kecamatan" />
                                </div>
                            </div>
                            ...
                            {data.length === 0 ? (
                                ...
                                data.map((item) => (
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell>
                                            <div className="text-sm">{item.jadwal_hari || '-'}</div>
                                            <div className="text-xs text-muted-foreground">{item.waktu || '-'}</div>
                                        </TableCell>
